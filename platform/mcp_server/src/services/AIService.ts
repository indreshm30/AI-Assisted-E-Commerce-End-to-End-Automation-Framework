...
export class AIService {
  ...
  public async generateCompletion(request: AIServiceRequest): Promise<AIServiceResponse> {
    ...
    try {
      ...
      if (request.provider === 'local' && this.localAIService) {
        response = await this.localAIService.generateCompletion(request);
        ...
        if (this.localAIService) {
          logger.info('Requested provider unavailable, falling back to local models');
          response = await this.localAIService.generateCompletion({
            ...request,
            provider: 'local'
          });
          ...
          logger.info('No providers available, generating mock response for development');
          response = this.generateMockResponse(request, requestId, startTime);
        }
      }
      ...
      const errorResponse: AIServiceResponse = {
        ...
        content: '',
        responseTime,
        model: request.model || (request.provider === 'openai' ? config.ai.openai.model : config.ai.anthropic.model),
        provider: request.provider,
        requestId,
  ...
  private generateMockResponse(request: AIServiceRequest, requestId: string, startTime: number): AIServiceResponse {
    const mockResponses = {
      'test-generation': `// Generated Test Code (MOCK for development)
describe('${request.context?.targetFunction || 'targetFunction'}', () => {
    ...
    "issues": [
      {
        "type": "warning",
        "severity": "medium", 
        "message": "Mock validation result - configure local AI models for real analysis"
      }
    ]
  },
  "recommendations": [
    "Install local AI models (Qwen2.5-Coder) for real business rule validation",
  ...
  "codeQuality": {
    ...
    "issues": [
      {
        "type": "suggestion",
        "message": "Mock analysis - install local AI models for detailed code review"
      }
    ]
  },
  "recommendations": [
    "Set up local Qwen2.5-Coder model for comprehensive code analysis",
    "Consider using LM Studio or Ollama for local AI inference"
  ]
}`
    };

    const content = mockResponses[request.requestType] || 'Mock AI response - configure local models for real AI-powered analysis';
...
Output Format: Provide structured analysis with categorized findings and prioritized recommendations.`
  ...
  // Helper methods for checking available providers
  public getAvailableProviders(): string[] {
    const providers: string[] = [];
    
    if (this.openaiClient) providers.push('openai');
    if (this.anthropicClient) providers.push('anthropic');
    if (this.localAIService) providers.push('local');
    
    return providers;
  }

  public async getLocalModelStatus(): Promise<{ [key: string]: boolean }> {
    if (!this.localAIService) {
      return {};
    }
    return await this.localAIService.checkModelAvailability();
  }
}